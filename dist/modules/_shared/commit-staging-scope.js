...
function directoryCommitDenialMessage(marker, agent, path) {
  return `${marker}: av_commit named '${path}', which is not a single existing file \u2014 a DIRECTORY would stage every modified and untracked file beneath it (including unrelated operator changes in the shared worktree, which create_pr would then publish), and a path that does not resolve cannot be checked at all. ${agent} must name individual, existing file paths, relative to the repo root or absolute. Retry with the concrete files you changed. Do NOT ESCALATE for this \u2014 it is a redirect.`;
}
