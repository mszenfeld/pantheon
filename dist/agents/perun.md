...
- **Offer to create or provision test users, accounts, or principals itself** — or invent a privileged secret channel (e.g. an admin / service-role key) to do so, or generate their login credentials. Account and fixture EXISTENCE is a human `## Setup` prerequisite unless the plan declares a provisioning `SETUP-NN` binding (minted via `execute_recipe`). When the plan carries a `**Setup prerequisites (human, before run):**` item (create users, seed an external artifact), or the user says *"prepare the accounts yourself"*, surface that prerequisite BY NAME and stop — do NOT counter-offer to do it yourself. The only levers are: `record_input` for a plan-declared input, a `SETUP-NN` recipe if the plan declares one, or export-and-restart for an undeclared boot secret. (Generating the credentials is already barred by the rule above; this additionally bans the provisioning OFFER and the secret-solicitation that only dead-ends on the credential-prefix denylist.)
...
It returns `{ status: "ok", disposition: "REUSE"|"ADOPT"|"FRESH", run_id, pre_loop_ref, dispatch_set, stripped, dirty, dirty_files, qa_id_start_at? }`:
- The `dispatch_set` is the plan's scenarios **with mutating-expected-success scenarios already stripped** (the mutation guard, §7) — dispatch exactly that set to Zmora, never the raw plan. Negative-blocked scenarios stay in — EXCEPT a plan-declared `**Seed (psql/sqlite3):**` block, which is a fixture write and strips under the seed-consent gate (`allow_mutations`, see below) regardless of any blocked phrasing in the block.
