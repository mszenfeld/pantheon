...
describe("preflight handler", () => {
  it("returns ok when every env name is set in the process env", async () => {
    const { handler } = makeDeps({ TEST_USER_EMAIL: "a@b.c", BASE_URL: "http://localhost:8000" })
  ...
  it("treats a name bound in the store (user-paste) as present, even when absent from process env", async () => {
    ...
    store.writeBinding("perun1", "TEST_USER_EMAIL", "a@b.c", "secret", "user-paste", { declaredInput: true })
  ...
  it("reports names absent from BOTH the store and the process env as missing", async () => {
    ...
    const result = await handler({ env: ["TEST_USER_EMAIL", "SUPABASE_URL", "SUPABASE_ANON_KEY"] }, ctx)
    expect(result).toEqual({ status: "missing", missing: ["SUPABASE_URL", "SUPABASE_ANON_KEY"] })
  })
