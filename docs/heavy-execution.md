...
7. **Publish/branch carve-out (executor-chain doctrine, 2026-07-22):** `create_pr` (validated push + PR — `docs/specs/create-pr-tool.md` §5.5) and `create_branch` (convention-validated branch creation — `docs/specs/create-branch-tool-2.md` §5.4) are **allowed before the immutable floor** — which would otherwise deny them via its `create_` verb pattern. `av_commit` needs no such carve-out to pass — under the allow-by-default design it is not floor-denied to begin with — and its access is sanctioned by the same 2026-07-22 executor-chain decision (`docs/specs/create-pr-tool.md` §9 item 5) that authorizes the self-serve publish chain `create_branch` → `av_commit` → `create_pr`. It is, however, **fail-closed on staging scope** (`src/modules/_shared/commit-staging-scope.ts`), because a bare `av_commit` falls back to `git add -A` and would commit the operator's unrelated working-tree changes — which `create_pr` then publishes. Two conditions must hold: (a) `files` is a non-empty list of concrete paths — a missing list, a whole-tree pathspec (`.`, `./`, `/`), git pathspec magic (`:/`), a glob or a `..` traversal is refused; and (b) no named path is a directory, checked on disk against the same worktree the tool stages with, since `git add -- src` would otherwise add everything modified or untracked beneath it. A path that cannot be resolved is refused too (it cannot be proven to be a single file). The named paths also bound the commit itself, not just the staging step: `createControlledCommit` passes them as a pathspec to `git commit`, so an index widened out-of-band (a bash `git add -A`, or work the operator staged before the dispatch) cannot ride along. Svarog is deliberately **not** held to Stribog's exact-membership rule: it has no edit budget and legitimately commits generated output it never hand-edited, such as this repo's tracked `dist/` tree.
...
**Inside the QA loop, restore is automatic.** When Svarog runs as the in-loop fixer, `qa_loop_record_fix` resolves `refs/svarog/ckpt/<sessionID>` from the child session id surfaced on `DispatchResult.sessionId` and, on `FAIL`, auto-restores that checkpoint (carrying only `READY` fixes forward). The manual `git for-each-ref` path remains for standalone Workflow-3 Svarog runs. **The automatic restore is bounded the same way the manual one is** (see [Honest limits](#honest-limits)): it restores the working tree, never commits. A fixer that committed its verified-green work via `av_commit` leaves those commits in place across both the `FAIL` restore and the total undo — reverting them is an operator step (`git reset --hard <pre-loop HEAD>`, or the reflog). Note also that `createCheckpoint`'s `update-ref` is **create-only** (a freshness guard): a host-restart-resumed session that re-fires the hook keeps its ORIGINAL pre-edit checkpoint instead of overwriting it.
