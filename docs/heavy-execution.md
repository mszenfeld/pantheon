...
**Svarog** is a heavy/main code executor dispatched by the Perun coordinator. Perun hands it a goal — often a Veles plan — and Svarog executes it end-to-end: explores the codebase, writes code test-first, runs the full suite/build, and returns a verified diff. Svarog is a leaf: it never delegates or spawns other agents, and it never produces secrets or commits.
