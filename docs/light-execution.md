...
- **Tool-name allow-list with an immutable capability-deny layer.** For a confirmed stribog session the hook decides **in this order** (the order is load-bearing):
  ...
  3. **Commit-module carve-out (executor-chain doctrine, 2026-07-22):** after attribution and the serena handling, three sanctioned commit-plugin tools are allowed, unbudgeted, for a positively-attributed stribog session — `create_pr` (validated push + PR; `docs/specs/create-pr-tool.md` §5.5), `create_branch` (convention-validated branch creation; `docs/specs/create-branch-tool-2.md` §5.4), and `av_commit` (controlled commit — not itself floor-denied, so its carve-out exempts it from the step-5 allow-list gate rather than from the floor). This is the harness-side enforcement of the full self-serve publish chain `create_branch` → `av_commit` → `create_pr` (recorded decision, `docs/specs/create-pr-tool.md` §9 item 5, MoA finding ARCH-001). Bash `git commit`/`git push` remain blocked plugin-wide for everyone.
  4. **Immutable capability-deny (`isImmutableDeny`)** — applied to everything that reaches the attribution gate, **winning over any `extraTools` config**. It denies these capability classes by name or segment-anchored regex: secret-minting (`execute_recipe`); leaf-dispatch (`task` + the dispatch family `dispatch_parallel`/`dispatch_background`/`poll_background`/`wait_background`, plus the `dispatch_*` / `*_task` patterns); shell/exec (`*_execute_shell`, `*_shell_command`); and code/state writes (`serena_write_memory`, `serena_replace_symbol_body`, `serena_replace_content`, `serena_create_text_file`, etc.). **No config can re-enable these** — this makes `execute_recipe` and the `serena`-write family a *pattern-enforced guarantee*, not the incidental result of a closed allow-list.
  5. `edit`/`write` (the remaining core builtins) are allowed and fall through to the edit budget; any tool matching a configured `extraTools` pattern (and not immutably denied) is allowed in the same trust class as `bash`.
  6. Any tool not covered by the above is refused with `STRIBOG_TOOL_DENIED`.
