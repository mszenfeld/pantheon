...
2. **Only** `read`/`glob`/`grep`/`edit`/`write`/`bash`, plus the sanctioned publish-chain tools `create_branch`/`av_commit`/`create_pr` (`av_commit` only with a `files` list naming paths this session actually edited) — any other tool is refused with `STRIBOG_TOOL_DENIED` (enforced).
...
- **Tool-name allow-list with an immutable capability-deny layer.** For a confirmed stribog session the hook decides **in this order** (the order is load-bearing):
  1. **Pre-filter:** the four non-edit core builtins (`read`, `glob`, `grep`, `bash`) pass immediately, *before* attribution — they can have no effect on the boundary, so the hook skips the attribution fetch for them. (Only these four short-circuit here; the other two `CORE_BUILTINS`, `edit`/`write`, are deliberately excluded from the pre-filter so they fall through to the attribution gate and the edit budget — see step 4.)
  2. **Attribution gate:** the hook resolves the session's agent and **fails open** for any non-stribog/unresolved session. This is why every *denial* is attribution-gated — a denied id (e.g. `execute_recipe`) is never refused for another agent that legitimately uses it (`zmora-setup`).
     - **Commit-module carve-out (executor-chain doctrine, 2026-07-22)** — step `(2d)` in the hook's own order map, between the attribution/serena handling and the floor. Three sanctioned commit-plugin tools are allowed, unbudgeted, for a positively-attributed stribog session: `create_pr` (validated push + PR; `docs/specs/create-pr-tool.md` §5.5), `create_branch` (convention-validated branch creation; `docs/specs/create-branch-tool-2.md` §5.4), and `av_commit` (controlled commit — not itself floor-denied, so its carve-out exempts it from the step-4 allow-list gate rather than from the floor). `av_commit` is additionally **fail-closed on staging scope** (`src/modules/_shared/commit-staging-scope.ts`), in two layers, because a bare call falls back to `git add -A` and would sweep the operator's unrelated changes into the executor's commit — which `create_pr` then publishes. (a) **Shape gate:** `files` must be a non-empty list of concrete paths; a missing list, a whole-tree pathspec (`.`, `./`, `/`), git pathspec magic (a leading `:`, e.g. `:/`), a glob, or a `..` traversal is refused. (b) **Exact membership (Stribog-specific):** every named path must be one this session actually edited — compared as resolved absolute paths against the `STRIBOG_EDIT_BUDGET` set, so a commit can never reach past the 2-file blast radius the budget bounds, and a bare basename never satisfies a deeper edited path. Both refusals are `STRIBOG_SCOPE_VIOLATION`. `av_commit` is unbudgeted only in the sense that it consumes no new budget slots — it is strictly bounded by the slots already consumed. The bound reaches the commit itself, not just the staging step: `createControlledCommit` passes the same paths as a pathspec to `git commit`, so an index widened out-of-band (a bash `git add -A`, which the mutating-git tripwire does not cover, or work the operator staged before the dispatch) cannot ride along. Together these enforce the self-serve publish chain `create_branch` → `av_commit` → `create_pr` (recorded decision, `docs/specs/create-pr-tool.md` §9 item 5). Bash `git commit`/`git push` remain blocked plugin-wide for everyone.
  3. **Immutable capability-deny (`isImmutableDeny`)** — applied to everything that reaches the attribution gate, **winning over any `extraTools` config**. It denies these capability classes by name or segment-anchored regex: secret-minting (`execute_recipe`); leaf-dispatch (`task` + the dispatch family `dispatch_parallel`/`dispatch_background`/`poll_background`/`wait_background`, plus the `dispatch_*` / `*_task` patterns); shell/exec (`*_execute_shell`, `*_shell_command`); and code/state writes (`serena_write_memory`, `serena_replace_symbol_body`, `serena_replace_content`, `serena_create_text_file`, etc.). **No config can re-enable these** — this makes `execute_recipe` and the `serena`-write family a *pattern-enforced guarantee*, not the incidental result of a closed allow-list.
  4. `edit`/`write` (the remaining core builtins) are allowed and fall through to the edit budget; any tool matching a configured `extraTools` pattern (and not immutably denied) is allowed in the same trust class as `bash`.
  5. Any tool not covered by the above is refused with `STRIBOG_TOOL_DENIED`.
- **Edit budget.** At most `STRIBOG_EDIT_BUDGET` (2) distinct files via `edit`/`write`; a third is refused with `STRIBOG_SCOPE_VIOLATION`. The budget keys on resolved **absolute** paths, so a native `edit`/`write` whose `filePath` is missing or non-absolute is refused with `STRIBOG_SCOPE_VIOLATION` (fail-closed) — it cannot be bound to the budget. The budget binds **only** native `edit`/`write`: write-capable `extraTools` are **denied** by the `isImmutableDeny` capability floor, never budgeted (the hook keeps no per-file accounting for MCP writes).
...
> **UPGRADE TRIPWIRE — verified against opencode 1.17.3 (binary-checked).** opencode forms MCP tool ids as `<serverKey>_<toolName>` and **preserves dashes**: its id sanitizer is `replace(/[^a-zA-Z0-9_-]/g, "_")`, so `-` is a legal id character and a kebab-case tool stays kebab-case (e.g. `context7_resolve-library-id`). The capability-deny matcher's correctness is **conditioned on this dash-preserving behavior** — because dashes survive, `isImmutableDeny` normalizes `-`→`_` before matching its underscore-segmented patterns (the SEC-001 fix). **If a future opencode release changes how MCP tool ids are formed or sanitized** (stops preserving dashes, changes the `_` separator, or alters the sanitizer regex), **the deny matcher and its `-`→`_` normalization must be re-validated.** The matcher lives in `isImmutableDeny` in `src/modules/_shared/stribog-extra-tools-contract.ts` (re-exported via `src/modules/stribog/stribog.metadata.ts`).
...
> **REQUIRED precondition for any `supabase_*`-class (or other DB/data) `extraTools` grant.** Granting Stribog a database or data MCP tool (`supabase_execute_sql`, any `*_query`/`*_sql`/datastore reader, etc.) **requires a read-restricted, least-privilege DB role**, scoped to exactly the tables the task needs. This is **not** optional hardening: the `tool.execute.before` hook gates which tool ids run (allow/deny) but **does not transform or scrub the results** — DB-tool results are returned to model context **verbatim**, so a `SELECT *` over a secret-bearing table (`auth.users`, service-role credential rows) puts those values straight into the model's context. The connection's own grants are therefore the **only** control on what data the tool can surface, and the least-privilege role is the compensating control. (Result scrubbing is `zmora-*`-only — see [§3.6 of the design spec](superpowers/specs/2026-06-14-stribog-extra-tools-design.md) — and a Stribog result-scrubber/column-denylist is tracked as a forward-looking follow-up, not a substitute for the role.)
...
Unlike Triglav (which inherits the session default), **Stribog is a doer and pins an explicit default**: `opencode-go/kimi-k2.7-code` (`DEFAULT_STRIBOG_MODEL` in `src/modules/stribog/stribog.metadata.ts`). This is a role + cost fit, **not** a security control — the security boundary is the tool-budget hook, never the model choice.

The pick is **evidence-based**: the 2026-06-16 eval (`docs/eval/scenarios/stribog/`, kimi-k2.7-code vs the prior `openai/gpt-5.4` default, run per `docs/eval/playbook.md`) found that — after the collision / secret-gate / serena fixes — `opencode-go/kimi-k2.7-code` passes all three discipline gates **natively** (instant zero-tool `ESCALATE` on out-of-scope work, value-free secret hand-off naming `zmora-setup`, curl-verified `FAIL` on the false-READY liveness trap) and produces clean, pattern-correct edits via native `Edit`/`Write` — at ~3× lower cost than `openai/gpt-5.4`. `openai/gpt-5.4` is now also fully functional after the same fixes (it edits via serena) and remains a solid override when you prefer it. An earlier 2026-06-09/10 eval had picked `openai/gpt-5.4` because the then-untested opencode-go models failed gates that the Stribog-side fixes have since closed. Re-run the scenarios per `docs/eval/playbook.md` before changing this default.
