...
2. **Only** `read`/`glob`/`grep`/`edit`/`write`/`bash`, plus the sanctioned publish-chain tools `create_branch`/`av_commit`/`create_pr` (`av_commit` only with an explicit `files` list) — any other tool is refused with `STRIBOG_TOOL_DENIED` (enforced).
