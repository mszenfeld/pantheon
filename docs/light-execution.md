...
- **Tool-name allow-list with an immutable capability-deny layer.** For a confirmed stribog session the hook decides **in this order** (the order is load-bearing):
  ...
  2d. **Commit-module carve-out (executor-chain doctrine, 2026-07-22):** after attribution and the serena handling — and keeping the hook's own step label (`(2d)` in `tool-budget-hook.ts`) — three sanctioned commit-plugin tools are allowed, unbudgeted, for a positively-attributed stribog session: `create_pr` (validated push + PR; `docs/specs/create-pr-tool.md` §5.5), `create_branch` (convention-validated branch creation; `docs/specs/create-branch-tool-2.md` §5.4), and `av_commit` (controlled commit — not itself floor-denied, so its carve-out exempts it from the step-4 allow-list gate rather than from the floor). This is the harness-side enforcement of the full self-serve publish chain `create_branch` → `av_commit` → `create_pr` (recorded decision, `docs/specs/create-pr-tool.md` §9 item 5). Bash `git commit`/`git push` remain blocked plugin-wide for everyone.
