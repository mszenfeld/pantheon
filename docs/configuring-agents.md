...
| `veles` | `Veles - Planner` (mode `all`) | Planning specialist. Works in three modes — **feature spec**, **implementation plan**, and **QA test plan** — authoring durable artefacts from a diff or request; dispatches read-only helpers. Inherits the session default model when `agents.veles.model` is unset. | Yes — via `pantheon.json` |
...
| `svarog` | `svarog` (subagent) | Heavy/main code executor. Implements a multi-file feature or refactor from a plan — writes code test-first, runs the full suite/build, and returns a verified diff. Finishes at READY; commits go only through the sanctioned `av_commit` tool (never bash `git commit`). Like Stribog, **pins an explicit default** (`openai/gpt-5.5`, the strongest GPT on the OpenAI subscription) when `agents.svarog.model` is unset (see note below). | Yes — via `pantheon.json` |
...
>
> For details on Veles's three modes — feature spec, implementation plan, and QA test plan — see [Veles planning](veles-planning.md).
