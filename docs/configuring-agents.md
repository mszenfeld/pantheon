...
> **Stribog model defaults.** Unlike the other agents — which inherit the session default model when their entry is unset — Stribog **pins a Sonnet-class default** (`anthropic/claude-sonnet-4-6`) when `agents.stribog.model` is not set. It is a doer that performs real side effects, so it defaults to a more capable model rather than cheap retrieval. Setting `agents.stribog.model` overrides this default. This pinned default is a **tier hint, not a security control** — it does not gate or restrict what Stribog can do.
