...
- **Any scenario you write that needs an authenticated call → declare its credential** (keyed on the SCENARIOS, not only the diff — a BE-only diff with no new `os.environ[...]` still needs this when its endpoints are guarded). A scenario emitting `Authorization: Bearer <token>`, an API-key header, or a session cookie MUST have that credential declared under `## Setup`: a **static** human-supplied token → a `**Required environment variables:**` NAME the header references by `$VAR` (never the literal `<token>`); a **dynamic** token minted at login → a `**Bindings:**` `QA_BIND_*` entry (Step 4.6 authority grounding). Emitting a `Bearer <token>` header with NO matching Setup declaration is a plan defect: `preflight([])` passes trivially on the empty list, so the missing credential only surfaces mid-run as `NEED_INFO` across every guarded scenario — see `test-plan-format` Setup Rules ("A scenario auth credential MUST be declared in `## Setup`").
...
**Account existence is a human prerequisite that preflight cannot verify.** A binding's login credentials name an account that must ALREADY EXIST in the auth system before the recipe can mint a token — but `preflight` checks only that the credential VALUES are present, never that the account exists (a pasted `TEST_USER_B_PASSWORD` passes preflight even if user B was never created, and the recipe then fails at login: credential-presence ≠ account-existence). The runner's four tools cannot create an auth account WITHOUT a privileged admin key the coordinator must never mint on its own, so account existence DEFAULTS to a human precondition. Express it as a first-class, imperative `## Setup` prerequisite naming the exact human action (e.g. `Create test users A and B in local Supabase Auth with confirmed emails: <exact command>`) — the same "name the command" rule Step 4.5 applies to a Required service. NEVER phrase it as optional prose ("create … if necessary") or bury it in a free-text block: that reads as agent-doable and invites an unsupported auto-provision attempt. If the account cannot be produced by the four tools, the scenarios that depend on it are **provisioning-blocked** (Step 6.7) and carry that Setup prerequisite.

**Escape — a provisioning recipe.** When the account CAN be created by an egress-validated `curl` admin-API call AND the operator will supply the privileged admin key + consent, declare a **provisioning binding** instead of punting to a human: a `**Bindings:**` entry carrying a `- Provisions: <principal>` marker (test-plan-format §Field rules), with the admin key (e.g. `$SUPABASE_SERVICE_KEY`) as a declared `Inputs:` name. That turns the dependent surface from `provisioning-blocked` into `covered`, run under the `allow_provisioning` consent gate — the ONLY sanctioned auto-provisioning path (the key stays user-supplied and plan-declared; the coordinator never mints it). Absent such a recipe, account existence stays a human prerequisite.
