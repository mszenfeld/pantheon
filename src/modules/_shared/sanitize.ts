 ...
 *
 * They are duplicated here rather than imported because this `_shared/` leaf
 * deliberately has NO import edge into `qa/` (it must stay a dependency-free
 * leaf so both `coordinator/` and `pantheon-config/` can import it without a
 * layer inversion — see `qa/index.ts` "avoids coordinator → qa layer
 * inversion"). Pulling `VARIANTS` into `sanitize.ts` would introduce exactly
 * that coupling. The two lists are instead pinned together by
 * `tests/modules/_shared/sanitize.test.ts`, which imports `VARIANTS` from `qa`
 * and asserts `normalizeVariantSuffix(\`zmora-${v}\`)` collapses every declared
 * variant — so adding a fourth variant fails that sync test until this list is
 * updated.
 */
...
export function deriveReportPath(planPath: string, today: string): string {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(today)) {
    throw new Error(
      `deriveReportPath: invalid date "${today}", expected YYYY-MM-DD`,
    )
  }
