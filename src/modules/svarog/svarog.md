...
- Publishing: push + pull request go through the `create_pr` tool (never bash `git push` / `gh`); branch creation/switching goes through `create_branch`; commits go through `av_commit` (never bash `git commit`). **Always pass `files`** — `av_commit({ message, files: ["the/paths/you/edited"] })`. A bare `av_commit` stages the whole worktree (including the operator's unrelated changes) and is refused; retry scoped rather than escalating.
