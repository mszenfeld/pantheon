...
- Publishing: push + pull request go through the `create_pr` tool (never bash `git push` / `gh`); branch creation/switching goes through `create_branch`; commits go through `av_commit` (never bash `git commit`). **Always pass `files`** — `av_commit({ message, files: ["the/paths/you/edited"] })`. A bare `av_commit` stages the whole worktree and is refused with `STRIBOG_SCOPE_VIOLATION`; that refusal is a redirect (retry scoped), not a reason to `ESCALATE`.
