...
**Until that lands, the import direction is FROZEN:** no *new* `src/modules/` file may import from `@appverk/opencode-skill-utils`. The only grandfathered consumers are `coordinator-policy/` (the bash gate + `read-allowlist.ts`), `stribog/` (session identity), and `svarog/` (session identity — `getSessionAgentCached` for its tool-hook attribution, same as stribog). This freeze is restated as a hard constraint in [Adding a New Absorbed Module](#adding-a-new-absorbed-module).
