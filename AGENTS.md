...
- Perun's local `av_commit` exception is enforced in the commit module: runtime caller
  ...
  rather than caller-selectable, and `create_branch` / `create_pr` reject Perun before any
  Git or provider call. Perun's `av_commit: true` and publication-denial tool-map entries are
  declarative defense in depth only.
