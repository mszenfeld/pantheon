...
| `src/modules/commit/` | Absorbed commit plugin — TS source only. Registers the sanctioned git-mutation surface as three plugin tools: `av_commit` (controlled commit), `create_branch` (convention-validated branch creation + same-commit checkout), and `create_pr` (validated push + `gh`-backed PR creation, with partial-success reporting) — together the executor publish chain. `create_branch`/`create_pr` match the hooks' immutable-deny `create_` verb and are allowed by named, attribution-gated carve-outs; `av_commit` is not floor-denied and is carved out of Stribog's allow-list gate instead, and both hooks refuse it unless it names concrete files to stage (`_shared/commit-staging-scope.ts`). Bash `git commit`/`git push` stay blocked by `bash-policy.ts`. Specs: `docs/specs/create-branch-tool-2.md`, `docs/specs/create-pr-tool.md`. Asset: `src/commands/commit.md`. Tests: `tests/modules/commit/`. Built into `dist/modules/commit/` and `dist/commands/`. |
