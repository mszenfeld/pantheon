...
| `src/modules/commit/` | Absorbed commit plugin — TS source only. Registers the sanctioned git-mutation surface as three plugin tools: `av_commit` (controlled commit), `create_branch` (convention-validated branch creation + same-commit checkout), and `create_pr` (validated push + `gh`-backed PR creation, with partial-success reporting) — together the executor publish chain, allowed past the executor hooks' immutable floor by named, attribution-gated carve-outs. Bash `git commit`/`git push` stay blocked by `bash-policy.ts`. Specs: `docs/specs/create-branch-tool-2.md`, `docs/specs/create-pr-tool.md`. Asset: `src/commands/commit.md`. Tests: `tests/modules/commit/`. Built into `dist/modules/commit/` and `dist/commands/`. |
