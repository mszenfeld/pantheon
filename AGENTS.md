...
| `src/modules/stribog/` | Absorbed light-executor module — TS source only. Registers the `stribog` actuator subagent (`mode: "subagent"`) and calls `registerAgentMetadata()` so Perun can route to it; pins a Sonnet-class default model (overridable via `agents.stribog.model`). Deny-by-default actuator allow-list: Read/Glob/Grep + Edit/Write + Bash for docker / make / package-managers (npm/pnpm/bun/uv) / curl + read-only git (`log`/`blame`/`status`/`diff`). Intentionally has **no** `execute_recipe`/secret-minting (minter ≠ actuator — that stays with `zmora-setup`), **no** dispatch/`Task` (it is a leaf, never fans out), and **no** `rm`. Asset: `stribog.md`. Tests: `tests/modules/stribog/`. Built into `dist/modules/stribog/`. |
